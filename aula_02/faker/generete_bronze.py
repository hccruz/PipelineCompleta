...
# Ativos (inclui BTC e 4 commodities)
ASSETS = [
    ("BTC", "Bitcoin", "coin", "USD"),
    ("GOLD", "Gold", "oz", "USD"),
    ("OIL", "WTI Oil", "bbl", "USD"),
    ("COFFEE", "Coffee", "kg", "USD"),
...
def daterange_days(start: date, end: date):
    for dt in rrule(DAILY, dtstart=datetime(start.year, start.month, start.day),
...
def build_customers(n=NUM_CLIENTS):
    ...
    for i in range(1, n + 1):
        rows.append({
            ...
            "documento": fake.cnpj(),
            "segmento": random.choice(["Financeiro", "Varejo", "Indústria", "Serviços", "Tecnologia"]),
            "pais": random.choice(["Brasil", "Estados Unidos", "Alemanha"]),
            "estado": fake.estado_sigla(),
            "cidade": fake.city(),
            "created_at": fake.date_time_between(start_date="-3y", end_date="-1y", tzinfo=timezone.utc),
        })
...
# =========================
def build_sales_btc_excel(customers_df, start=START_DATE, end=END_DATE):
    rows = []
    tx_id = 1
    for d in daterange_days(start, end):
        ...
        for _ in range(btc_today):
            ...
            rows.append({
                ...
                "ativo": "BTC",
                "quantidade": qty,
                "tipo_operacao": pick_tipo_operacao(),  # NOVO
                "moeda": "USD",
                "cliente_id": cliente,
    ...
    cols = ["transaction_id", "data_hora", "ativo", "quantidade", "tipo_operacao",
            "moeda", "cliente_id", "canal", "mercado", "arquivo_origem", "importado_em"]
    return df[cols]

# =========================
# 3) bronze_sales_commodities_sql (simplificada, sem preço; com tipo_operacao)
# =========================
def build_sales_commodities_sql(customers_df, start=START_DATE, end=END_DATE):
    rows = []
    tx_id = 1
    ...
    for d in daterange_days(start, end):
        ...
        for _ in range(comm_today):
            ...
            tx_id += 1
    df = pd.DataFrame(rows)
    cols = ["transaction_id", "data_hora", "commodity_code", "quantidade", "tipo_operacao",
            "unidade", "moeda", "cliente_id", "canal", "mercado", "arquivo_origem", "importado_em"]
    return df[cols]
